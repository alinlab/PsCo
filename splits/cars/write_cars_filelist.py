...
cwd = os.getcwd()
data_path = join(cwd,'/data/cars/cars_train')
savedir = './'
dataset_list = ['base','val','novel']

data_list = np.array(loadmat('/data/cars/devkit/cars_train_annos.mat')['annotations'][0])
class_list = np.array(loadmat('/data/cars/devkit/cars_meta.mat')['class_names'][0])
classfile_list_all = [[] for i in range(len(class_list))]

for i in range(len(data_list)):
  folder_path = join(data_path, data_list[i][-1][0])
  classfile_list_all[data_list[i][-2][0][0] - 1].append(join('cars_train',data_list[i][-1][0]))
